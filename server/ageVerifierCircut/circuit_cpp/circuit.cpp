...
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 101],&lvar[3]); // line circom 10
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 101]); // line circom 16
...
if (!Fr_isTrue(&circuitConstants[2])) std::cout << "Failed assert in template/function " << myTemplateName << " line 46. " <<  "Followed trace of components: " << ctx->getTrace(myId) << std::endl;
...
// load src
Fr_add(&expaux[1],&signalValues[mySignalStart + 1],&circuitConstants[4]); // line circom 52
Fr_sub(&expaux[0],&expaux[1],&signalValues[mySignalStart + 2]); // line circom 52
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
// load src
Fr_sub(&expaux[0],&circuitConstants[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 100]); // line circom 54
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 1; i++){
...
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 1],&circuitConstants[2]); // line circom 94
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// need to run sub component
